...
import { getPageIdByPermalink, fetchPageDataById, fetchPageData } from "$lib/directus/fetchers";


export const getPageData = query("unchecked", async (permalink: string) => {
    ...
    try {
        let pageId = id;
        if (version && !pageId) {
            const foundPageId = await getPageIdByPermalink(permalink, token);
            pageId = foundPageId || '';
        }
